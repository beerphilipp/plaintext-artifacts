...
mkdir -p "${APK_DIR}" "${MANIFEST_OUTPUT_DIR}" \
...
echo ""
echo "=> Downloading ${APP_COUNT} randomly selected apps to ${APK_DIR} ..."
docker run --rm \
    -v "${SSH_KEY}:/root/.ssh/webview_key:ro" \
    -v "${APK_DIR}:/apks" \
    "${DOWNLOADER_IMAGE}" /apks "${APP_COUNT}" \
    >/dev/null 2>&1 || abort "Failed to download APKs."

echo "  APKs downloaded to ${APK_DIR}."
