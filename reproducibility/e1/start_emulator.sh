...
#
# Description:
#   Creates and launches an Android Virtual Device (AVD) named 'webview_emulator'
#   using the Pixel 6a device profile and system image for Android API level 36.
...
echo "📦 Installing/updating system image for Android 36 ($SYSTEM_ARCH)..."
echo "    You may be prompted to accept licenses ..."
sleep 5
if ! "$SDKMANAGER_PATH" "system-images;android-36;google_apis;$SYSTEM_ARCH"; then
  abort "Failed to install system image."
fi

echo "🛠️ Creating AVD named 'webview_emulator'..."
if ! "$AVDMANAGER_PATH" create avd -n webview_emulator \
    --force \
    -k "system-images;android-36;google_apis;$SYSTEM_ARCH" \
    --device "pixel_6a" > /dev/null; then
  abort "Failed to create AVD."
fi

echo "🚀 Starting emulator..."
if ! "$EMULATOR_PATH" -avd webview_emulator -writable-system -no-snapshot-load > /dev/null; then
